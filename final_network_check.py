...
def final_network_check():
    ...
    try:
        ...
        # Get subnet entities - check both subnet 0 and 1
        miners_0 = contract.functions.getSubnetMiners(0).call()
        validators_0 = contract.functions.getSubnetValidators(0).call()

        miners_1 = contract.functions.getSubnetMiners(1).call()
        validators_1 = contract.functions.getSubnetValidators(1).call()

        # Combine results
        miners = miners_0 + miners_1
        validators = validators_0 + validators_1
        ...
        print(
            f"  {'✅' if miners_ok else '❌'} Miners: {len(miners)}/2 ({'OK' if miners_ok else 'NEED MORE'})"
        )
        print(
            f"  {'✅' if validators_ok else '❌'} Validators: {len(validators)}/2 ({'OK' if validators_ok else 'NEED MORE'})"
        )

        if miners_ok and validators_ok:
            print(f"\n🎉 NETWORK READY FOR OPERATION!")
            print(f"  ✅ Sufficient entities registered")
            print(f"  ✅ ModernTensor architecture active")
