...
class FlexibleConfigManager:
    ...
    def _create_default_consensus_configs(self) -> Dict[str, FlexibleConsensusConfig]:
        ...
        return {
            ConsensusMode.RIGID.value: FlexibleConsensusConfig(
                slot_duration_minutes=3.5,
                min_task_assignment_seconds=60,
                min_task_execution_seconds=60,
                min_consensus_seconds=60,
                min_metagraph_update_seconds=30,
                task_deadline_buffer=0,
                consensus_deadline_buffer=0,
                metagraph_deadline_buffer=0,
                allow_mid_slot_join=False,
                auto_extend_on_consensus=False,
                max_auto_extension_seconds=0,
            ),
    ...
    def _create_default_miner_configs(self) -> Dict[str, FlexibleMinerConfig]:
        ...
        return {
            MinerMode.TRADITIONAL.value: FlexibleMinerConfig(
                ...
                result_submission_retries=2,
            ),
            
            MinerMode.ADAPTIVE.value: FlexibleMinerConfig(
                response_timeout_multiplier=1.2,
                task_execution_buffer=30,
                validator_detection_interval=10,
                auto_adjust_timing=True,
                max_concurrent_tasks=5,
                result_submission_retries=3,
            ),
            
            MinerMode.RESPONSIVE.value: FlexibleMinerConfig(
                response_timeout_multiplier=0.8,
                task_execution_buffer=15,
                validator_detection_interval=5,
                auto_adjust_timing=True,
            ...
            MinerMode.PATIENT.value: FlexibleMinerConfig(
                response_timeout_multiplier=1.5,
                task_execution_buffer=60,
                validator_detection_interval=15,
                auto_adjust_timing=True,
                max_concurrent_tasks=3,
                result_submission_retries=5,
            ),
        }
    ...
    def generate_startup_command(
        ...
        if entity_type == "validator":
            return (
                f"python scripts/run_flexible_validator_subnet1.py "
                f"--validator {entity_id} --mode {consensus_mode} --auto-adapt"
            )
        elif entity_type == "miner":
            return (
                f"python scripts/run_flexible_miner_subnet1.py "
                f"--miner {entity_id} --mode {miner_mode} --auto-detect-validators"
            )
        else:
