...
def register_all_entities():
    ...
    for entity_file in sorted(entities_dir.glob("*.json")):
        ...
        try:
            ...
            # Build transaction
            if entity_type == "miner":
                function = contract.functions.registerMiner(
                    core_stake, btc_stake, subnet_id
                )
            else:
                function = contract.functions.registerValidator(
                    core_stake, btc_stake, subnet_id
                )
            ...
            try:
                ...
                print(
                    f"  🔍 Explorer: https://scan.test.btcs.network/tx/{tx_hash.hex()}"
                )
    ...
    print(
        f"🔍 Check registrations at: https://scan.test.btcs.network/address/{contract_address}"
    )
