    ...
    for entity in entities:
        try:
            ...
            if not entity['private_key'] or not entity['address']:
                print(f"❌ Missing private key or address for {entity['type']} {entity['id']}")
                continue
                
            # Create account from private key
            account = web3.eth.account.from_key(entity['private_key'])
            if account.address.lower() != entity['address'].lower():
                print(f"❌ Private key doesn't match address for {entity['type']} {entity['id']}")
                continue
            ...
            signed_tx = web3.eth.account.sign_transaction(tx_data, entity['private_key'])
            ...
            if receipt.status == 1:
                print(f"   ✅ {entity['type'].title()} {entity['id']} registered successfully!")
