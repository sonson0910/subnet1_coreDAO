...
try:
    ...
    from sdk.metagraph.hash.hash_datum import hash_data # Dùng để hash wallet addr nếu cần
    ...
    from sdk.smartcontract.validator import read_validator # <<<--- Import read_validator
...
rich_handler = RichHandler(show_time=True, show_level=True, show_path=False, markup=True, rich_tracebacks=True, log_time_format="[%Y-%m-%d %H:%M:%S]")
logging.basicConfig(level=log_level, format="%(message)s", datefmt="[%X]", handlers=[rich_handler])
...
if env_path.exists():
    ...
    # <<<--- THÊM DEBUG --- >>>
    logger.debug(f"DEBUG: Value of HOTKEY_BASE_DIR from os.getenv AFTER load: {os.getenv('HOTKEY_BASE_DIR')}")
    # <<<----------------->>>
else:
...
# <<<--- THÊM DEBUG --- >>>
logger.debug(f"DEBUG: Final value assigned to HOTKEY_BASE_DIR variable: {HOTKEY_BASE_DIR} (Type: {type(HOTKEY_BASE_DIR)})")
# <<<----------------->>>
...
def load_funding_keys(
    ...
    try:
        ...
        funding_address = Address(payment_vk.hash(), stake_vk.hash() if stake_vk else None, network=get_network())
...
async def prepare_datums():
    ...
    logger.info("✨ --- Starting Metagraph Datum Preparation Script (using direct context/builder/read_validator) --- ✨")
    ...
    try:
        ...
        if not funding_coldkey:
             raise ValueError("FUNDING_COLDKEY_NAME is missing or empty in .env and no default specified.")
        if not funding_hotkey:
             raise ValueError("FUNDING_HOTKEY_NAME is missing or empty in .env and no default specified.")
        if not funding_password:
            raise ValueError("FUNDING_PASSWORD environment variable is not set.")
        if not hotkey_base_dir:
             raise ValueError("Could not determine HOTKEY_BASE_DIR (checked .env and sdk_settings).")
    ...
    try:
        logger.info(f"🔗 Initializing BlockFrostChainContext (Network: {network.name})...")
        context = get_chain_context(method="blockfrost") # type: ignore
        if not context: raise RuntimeError("Failed to initialize BlockFrost context.")
        if context.network != network:
             logger.warning(f"Context network ({context.network.name}) differs from settings ({network.name}). Using context network.")
             network = context.network

        min_utxo = context.protocol_param.min_utxo
        logger.info(f"💰 Minimum UTXO Value per Datum (Lovelace): {DATUM_LOCK_AMOUNT} (Protocol Min: {min_utxo})")
        if DATUM_LOCK_AMOUNT < min_utxo:
             logger.warning(f"⚠️ DATUM_LOCK_AMOUNT ({DATUM_LOCK_AMOUNT}) is less than protocol minimum ({min_utxo}). Transaction might fail.")
        ...
        if not validator_details or "script_hash" not in validator_details:
            raise RuntimeError("Could not load validator script hash using read_validator().")
        script_hash = validator_details["script_hash"]
        contract_address = Address(payment_part=script_hash, network=network) # Tạo địa chỉ từ hash
        ...
        logger.exception(f"💥 Error initializing Cardano context or reading validator script: {ctx_err}")
        sys.exit(1)
    ...
    try:
        ...
        if val1_uid and val1_addr and val1_api:
            validators_info.append(ValidatorInfo(uid=val1_uid.encode().hex(), address=val1_addr, api_endpoint=val1_api, status=STATUS_ACTIVE))
        ...
        if val2_uid and val2_addr and val2_api:
            validators_info.append(ValidatorInfo(uid=val2_uid.encode().hex(), address=val2_addr, api_endpoint=val2_api, status=STATUS_ACTIVE))
            logger.info(f"  ➕ Loaded Validator 2: UID={val2_uid}")
        else: logger.warning("⚠️ Validator 2 info missing in .env, skipping.")
        ...
        if val3_uid and val3_addr and val3_api:
            validators_info.append(ValidatorInfo(uid=val3_uid.encode().hex(), address=val3_addr, api_endpoint=val3_api, status=STATUS_ACTIVE))
            logger.info(f"  ➕ Loaded Validator 3: UID={val3_uid}")
        else: logger.warning("⚠️ Validator 3 info missing in .env, skipping.")
        ...
        if miner1_uid and miner1_addr and miner1_api:
             miners_info.append(MinerInfo(uid=miner1_uid.encode().hex(), address=miner1_addr, api_endpoint=miner1_api, status=STATUS_ACTIVE))
        ...
        if miner2_uid and miner2_addr and miner2_api:
             miners_info.append(MinerInfo(uid=miner2_uid.encode().hex(), address=miner2_addr, api_endpoint=miner2_api, status=STATUS_ACTIVE))
             logger.info(f"  ➕ Loaded Miner 2: ID={miner2_uid}")
        else: logger.warning("⚠️ Miner 2 info missing in .env, skipping.")

        if not validators_info and not miners_info:
            logger.critical("❌ FATAL: No validators or miners could be loaded from .env configuration. Cannot create datums.")
            sys.exit(1)
    ...
    try:
        ...
        for info in validators_info:
            try:
                ...
                val_datum = ValidatorDatum(
                    uid=bytes.fromhex(info.uid), subnet_uid=SUBNET_ID_TO_USE, stake=0,
                    scaled_last_performance=0, scaled_trust_score=0, accumulated_rewards=0,
                    last_update_slot=current_slot, performance_history_hash=b'',
                    wallet_addr_hash=wallet_hash_bytes, status=STATUS_ACTIVE,
                    registration_slot=current_slot,
                    api_endpoint=info.api_endpoint.encode('utf-8') if info.api_endpoint else b''
                )
                tx_out = TransactionOutput(contract_address, Value(DATUM_LOCK_AMOUNT), datum=val_datum) # Sử dụng contract_address đã lấy
                outputs_to_create.append(tx_out)
                total_output_value += DATUM_LOCK_AMOUNT
                logger.info(f"  📄 Prepared ValidatorDatum UTXO for UID: {info.uid[:10]}...")
            except Exception as datum_err:
                 logger.error(f"  ❌ Failed to create ValidatorDatum for UID {info.uid}: {datum_err}")
        ...
        for info in miners_info:
             try:
                ...
                tx_out = TransactionOutput(contract_address, Value(DATUM_LOCK_AMOUNT), datum=miner_datum) # Sử dụng contract_address đã lấy
                outputs_to_create.append(tx_out)
                total_output_value += DATUM_LOCK_AMOUNT
                logger.info(f"  📄 Prepared MinerDatum UTXO for UID: {info.uid[:10]}...")
             except Exception as datum_err:
                 logger.error(f"  ❌ Failed to create MinerDatum for UID {info.uid}: {datum_err}")
    ...
    logger.info(f"🔍 Searching for input UTXO >= {min_input_needed / 1_000_000:.6f} ADA in funding wallet...")

    selected_input_utxo = find_suitable_ada_input(context, str(funding_address), min_input_needed)

    if not selected_input_utxo:
        logger.error(f"❌ Could not find a suitable ADA-only UTxO with at least {min_input_needed / 1_000_000:.6f} ADA at {funding_address}.")
        logger.error("   Ensure the funding wallet has enough ADA in a single, simple UTxO.")
    ...
    try:
        ...
        tx_id_str = str(tx_id)
        logger.info(f"✅ Transaction submitted successfully! Tx Hash: [bold green]{tx_id_str}[/]")
        ...
        logger.exception(f"💥 Final error during transaction build/sign/submit: {final_err}")
        sys.exit(1)
