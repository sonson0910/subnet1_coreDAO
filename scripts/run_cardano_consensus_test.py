#!/usr/bin/env python3
"""
Script to run Cardano consensus test with 2 validators and detailed logging
"""
import asyncio
import subprocess
import sys
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('cardano_consensus_test.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class CardanoConsensusRunner:
    def __init__(self):
        self.processes = []
        self.log_files = []
        self.running = True
        ...
        try:
            with open(log_file_path, 'r') as f:
                ...
                while self.running:
                    ...
                    # Look for transaction IDs (64 character hex strings starting with 0x)
                    ...
                    # Look for scores
                        ...
                        logger.info(f"📈 [{timestamp}] [{process_name}] SCORE: {line}")
                    ...
                    elif any(keyword in line.lower() for keyword in ['task', 'miner', 'assignment']):
    ...
    def run_consensus_test(self, duration=600):  # 10 minutes default
        """Run the consensus test"""
        logger.info("=" * 80)
        logger.info("🚀 STARTING CARDANO MODERNTENSOR CONSENSUS TEST")
        logger.info("=" * 80)
        ...
        try:
            ...
            self.start_process(
                "test_cardano_validator2.py", 
                f"logs/cardano_validator2_{timestamp}.log",
                "Cardano Validator 2"
            )
            
            logger.info("🎯 All Cardano validators started! Monitoring for activity...")
            ...
            logger.error(f"Error in consensus test: {e}")
        finally:
            self.cleanup()
            logger.info("✅ Cardano consensus test completed!")

def main():
    """Main function"""
    runner = CardanoConsensusRunner()
    ...
    runner.run_consensus_test(duration)
