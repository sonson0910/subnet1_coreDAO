#!/usr/bin/env python3
"""
Aptos Miner Server Only - No Agent
...
def run_miner_server_only():
    ...
    logger.info("⛏️ --- Starting Aptos Miner Server Only (No Agent) --- ⛏️")
    ...
    if miner_id == "2":
        ...
        miner_readable_id = f"miner_{miner_id}"  # Use simple format to match validator expectation
    ...
    try:
        ...
        logger.info(f"▶️ Starting Subnet1Miner server for '{miner_readable_id}' (UID: {expected_uid_hex})...")
        ...
        logger.exception(f"💥 An unexpected error occurred during miner server startup: {e}")
    finally:
...
if __name__ == "__main__":
    try:
        logger.info("🚦 Starting miner server only...")
