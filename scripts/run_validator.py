...
def main():
    ...
    logger.info("--- Configuring Validator Runner for Subnet 1 (Instance 1) ---")

    try:
        # --- Đọc ID validator dạng chuỗi từ .env ---
        validator_readable_id = os.getenv("SUBNET1_VALIDATOR_UID")
        if not validator_readable_id:
            logger.error("FATAL: SUBNET1_VALIDATOR_UID (readable validator ID) is not set in .env.")
            sys.exit(1)
        ...
        try:
            ...
            logger.info(f"Derived On-Chain UID Hex from SUBNET1_VALIDATOR_UID ('{validator_readable_id}'): {expected_uid_hex}")
        except Exception as e:
            logger.error(f"FATAL: Could not encode SUBNET1_VALIDATOR_UID ('{validator_readable_id}') to derive UID: {e}")
            sys.exit(1)
        # -----------------------------------------
        ...
        runner_config: Dict[str, Any] = {
            ...
            "coldkey_name": os.getenv("SUBNET1_COLDKEY_NAME", "validator1"), # Tên coldkey riêng cho validator 1
            "hotkey_name": os.getenv("SUBNET1_HOTKEY_NAME"), # Tên hotkey riêng cho validator 1
            "password": os.getenv("SUBNET1_HOTKEY_PASSWORD"), # Mật khẩu riêng cho validator 1
        ...
        if not all(runner_config.get(k) for k in required_keys):
            ...
            raise ValueError(f"Missing required configurations in .env for Validator 1: {missing}")
