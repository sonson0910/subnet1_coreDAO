#!/usr/bin/env python3
"""
Aptos Validator 2 Runner Script for Subnet1
"""
...
async def run_validator2_process():
    """Async function to configure and run Subnet1 Validator 2 for Aptos."""
    logger.info("🛡️ --- Starting Aptos Validator 2 Configuration & Process --- 🛡️")
    ...
    validator_readable_id = "subnet1_validator_2"

    if not validator_private_key:
        logger.critical("❌ FATAL: VALIDATOR_2_PRIVATE_KEY is not set in config.env.")
        return
    
    logger.info(f"🆔 Using Validator 2: '{validator_readable_id}'")
    ...
    required_configs = {
        "VALIDATOR_2_PRIVATE_KEY": validator_private_key,
        "APTOS_NODE_URL": aptos_node_url,
        "APTOS_CONTRACT_ADDRESS": aptos_contract_address,
        "VALIDATOR_2_API_ENDPOINT": validator_api_endpoint
    }
    missing_configs = [k for k, v in required_configs.items() if not v]
    if missing_configs:
        logger.critical(f"❌ FATAL: Missing Validator 2 configurations in config.env: {missing_configs}")
        return

    logger.info("🏗️ --- Subnet 1 Validator 2 (Aptos Blockchain) Configuration --- 🏗️")
    ...
    try:
        logger.info(f"🔑 Loading Aptos account for Validator 2...")
        if not validator_private_key:
            raise ValueError("VALIDATOR_2_PRIVATE_KEY is required")
            
        # Create Aptos account from private key
        validator_account = Account.load_key(validator_private_key)
        logger.info(f"✅ Validator 2 Aptos account loaded successfully. Address: {validator_account.address()}")
        
    except Exception as key_err:
        logger.exception(f"💥 FATAL: Failed to load Aptos account for Validator 2: {key_err}")
        return

    # --- Initialize and run validator --- 
    try:
        logger.info("🛠️ Initializing Subnet1Validator 2 instance...")
        ...
        validator_instance = Subnet1Validator(
            ...
            consensus_mode="synchronized",  # Enable sequential consensus for coordination between validators
        ...
        logger.info("✅ Subnet1Validator 2 instance initialized.")

        # Run Validator
        logger.info(f"▶️ Starting Subnet1Validator 2 main loop for UID {expected_uid_hex}...")
        await validator_instance.run()
        logger.info("⏹️ Subnet1Validator 2 main loop finished.")

    except Exception as e:
        logger.exception(f"💥 An unexpected error occurred during validator 2 process startup or execution: {e}")
    finally:
        logger.info("🛑 Validator 2 process cleanup finished.")


# --- Main execution point --- 
if __name__ == "__main__":
    try:
        logger.info("🚦 Starting Validator 2 main asynchronous execution...")
        asyncio.run(run_validator2_process())
    except KeyboardInterrupt:
        logger.info("👋 Validator 2 process interrupted by user (Ctrl+C).")
    except Exception as main_err:
        logger.exception(f"💥 Critical error in validator 2 main execution block: {main_err}")
    finally:
