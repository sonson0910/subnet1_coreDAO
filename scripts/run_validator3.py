...
def main():
    ...
    logger.info("--- Configuring Validator Runner for Subnet 1 (Instance 3) ---")

    try:
        # --- Đọc ID validator dạng chuỗi từ .env ---
        validator_readable_id = os.getenv("SUBNET1_VALIDATOR_UID3")
        if not validator_readable_id:
            logger.error("FATAL: SUBNET1_VALIDATOR_UID3 (readable validator ID) is not set in .env.")
            sys.exit(1)
        ...
        try:
            ...
            logger.info(f"Derived On-Chain UID Hex from SUBNET1_VALIDATOR_UID3 ('{validator_readable_id}'): {expected_uid_hex}")
        except Exception as e:
            logger.error(f"FATAL: Could not encode SUBNET1_VALIDATOR_UID3 ('{validator_readable_id}') to derive UID: {e}")
            sys.exit(1)
        # -----------------------------------------
        ...
        runner_config: Dict[str, Any] = {
            ...
            "host": os.getenv("SUBNET1_API_HOST3", "127.0.0.1"),
            "port": int(os.getenv("SUBNET1_API_PORT3", 8003)),
            ...
            "hotkey_base_dir": os.getenv("HOTKEY_BASE_DIR3", getattr(sdk_settings, 'HOTKEY_BASE_DIR', 'moderntensor')),
            "coldkey_name": os.getenv("SUBNET1_COLDKEY_NAME3", "validator3"), # Tên coldkey riêng cho validator 3
            "hotkey_name": os.getenv("SUBNET1_HOTKEY_NAME3"), # Tên hotkey riêng cho validator 3
            "password": os.getenv("SUBNET1_HOTKEY_PASSWORD3"), # Mật khẩu riêng cho validator 3
        ...
        if not all(runner_config.get(k) for k in required_keys):
            ...
            raise ValueError(f"Missing required configurations in .env for Validator 3: {missing}")
