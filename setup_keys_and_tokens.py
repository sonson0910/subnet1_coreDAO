...
class Subnet1Setup:
    ...
    def setup_network_config(self):
        ...
        self.console.print(
            "\n[bold blue]🌐 CORE BLOCKCHAIN NETWORK CONFIGURATION[/bold blue]"
        )
    ...
    def create_or_load_keys(self, network_config: Dict[str, Any]):
        ...
        if action == "list":
            ...
            action = Prompt.ask(
                "Now what?", choices=["create", "load"], default="create"
            )
    ...
    def create_new_keys(self):
        ...
        try:
            ...
            if mnemonic:
                ...
                self.console.print(
                    f"🔑 [bold red]BACKUP THIS MNEMONIC:[/bold red] {mnemonic}"
                )
                ...
                self.console.print(
                    f"🔨 Creating validator hotkey '{validator_hotkey}'..."
                )
            ...
            if coldkey_info:
                ...
                if hotkeys:
                    ...
                    validator_hotkey = Prompt.ask(
                        "Enter validator hotkey name", default="validator_hotkey"
                    )
                    miner_hotkey = Prompt.ask(
                        "Enter miner hotkey name", default="miner_hotkey"
                    )
                else:
                    ...
                    self.wallet_manager.add_hotkey(
                        coldkey_name, validator_hotkey, password
                    )
                    self.wallet_manager.add_hotkey(coldkey_name, miner_hotkey, password)
    ...
    async def request_tokens(
        self, keys_info: Dict[str, str], network_config: Dict[str, Any]
    ):
        ...
        try:
            ...
            validator_info = self.wallet_manager.get_hotkey_info(
                keys_info["coldkey_name"], keys_info["validator_hotkey"]
            )
            miner_info = self.wallet_manager.get_hotkey_info(
                keys_info["coldkey_name"], keys_info["miner_hotkey"]
            )
            ...
            self.console.print(
                "💸 Manual token request required from Core blockchain faucet..."
            )
    ...
    async def check_balances(
        self, network_config: Dict[str, Any], validator_addr: str, miner_addr: str
    ):
    ...
    def display_summary(
        ...
        self.console.print(
            "1. Update CORE_CONTRACT_ADDRESS in .env with actual contract"
        )
...
async def main():
    ...
    setup.console.print(
        "This script will help you create keys and request tokens for testing\n"
    )
