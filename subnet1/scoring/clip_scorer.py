...
def calculate_clip_score(
    ...
    try:
        ...
        with torch.no_grad(): # Không cần tính gradient
            ...
            logger.debug(f"CLIP score calculated for prompt '{prompt[:30]}...': {final_score:.4f} (Cosine Sim: {cosine_sim:.4f})")
            return final_score
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.info(f"Score for prompt '{test_prompt1}' (should be relatively high): {score1:.4f}")

        score2 = calculate_clip_score(prompt=test_prompt2, image_bytes=img_bytes)
        logger.info(f"Score for prompt '{test_prompt2}' (should be relatively low): {score2:.4f}")
